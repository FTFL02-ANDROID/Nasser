...
public class ShoppingComplexSQLiteHelper extends SQLiteOpenHelper {
	// ICare Profile Table
	public static final String TABLE_SHOPPING_COMPLES = "shoppingcomples";
	...
	private static final String DATABASE_CREATE_PROFILE = "create table "
			+ TABLE_SHOPPING_COMPLES + "( " + COL_ID
			+ " integer primary key autoincrement, " + " "
			+ COL_NAME + " text not null," + " "
			+ COL_ADDRESS + " text not null," + " "
			+ COL_LATTITUDE + " text not null," + " "
			+ COL_LONGITUDE + " text not null," + " "
			+ COL_CLOSE_DAY + " text not null," + " "
			+ COL_OPEN_TIME + " text not null," + " "
	...
	public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
		...
		db.execSQL("DROP TABLE IF EXISTS " + TABLE_SHOPPING_COMPLES);
		onCreate(db);
	}
