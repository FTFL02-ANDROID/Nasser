...
public class VaccinationCustomAdapter extends BaseAdapter {

	/*********** Declare Used Variables *********/
	private Activity activity;
	private List<Vaccine> data = new ArrayList<Vaccine>();
	...
	public VaccinationCustomAdapter(Activity a, List<Vaccine> d,
			Resources resLocal) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (data.size() <= 0) {
			...
			Vaccine vaccine = (Vaccine) data.get(position);

		

			holder.name.setText("" + vaccine.getName());
			holder.specialization.setText("" + vaccine.getDate());
